...
public class MainFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		authButton.setReadPermissions(Arrays.asList("user_location",
				"user_birthday", "user_likes", "email", "age_range",
				"education", "gender", "hometown", "religion"));
	...
	private String buildUserInfoDisplay(GraphUser user) {
		...
		userInfo.append(String.format("Religion: %s\n\n",
				user.getProperty("religion")));

		// userInfo.append(String.format("Probable BestFriend: %s\n\n",
		// CallLogs.getBestFriend(getActivity())));

		JSONObject age = (JSONObject) user.getProperty("age_range");
		if (age != null && age.length() > 0) {
			userInfo.append(String.format("Probable Age(Min): %s\n\n",
					age.optInt("min")));
		}

		JSONArray hometown = (JSONArray) user.getProperty("hometown");
		if (hometown != null && hometown.length() > 0) {

			JSONObject home = hometown.optJSONObject(0);
			userInfo.append(String.format("Home: %s\n\n", home.toString()));
		}
